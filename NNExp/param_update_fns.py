...
class GdFixedParamUpdate:
    def fn(self, config, biases, weights, total_loss,
        ...
        weights_new = [np.clip(regularization * w + eta_batch * GdFixedParamUpdate.fixed_delta(avg_loss, wb, regularization * w), -100, 100)
                        for w, wb in zip(weights, w_batch)]
        biases_new = [np.clip(b + eta_batch * GdFixedParamUpdate.fixed_delta(avg_loss, bb, b), -100, 100)
                        for b, bb in zip(biases, b_batch)]
    ...
    def fixed_delta(avg_loss, delta, max_val):
        ...
        with np.errstate(divide='ignore', invalid='ignore', over='ignore', under='ignore'):
            fixed = np.nan_to_num(-avg_loss/delta)
            np.clip(fixed, -0.01, 0.01, fixed)
        return fixed